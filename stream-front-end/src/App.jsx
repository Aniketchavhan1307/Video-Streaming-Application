...
function App() {
  ...
  const [videoId, setVideoId] = useState(
    "6eb3e4dc-9083-403a-a31f-a4ccd273e43a"
  );
    ...
    <>
      ...
      <div className="flex flex-col  items-center space-y-9 justify-center py-9">
        ...
        <TextInput
                value={fieldValue || ""}
                onChange={(event) => {
                  setFieldValue(event.target.value); // Update on input change
                }}
              ...
              <Button
                onClick={() => {
                  if (fieldValue && fieldValue.trim().length > 0) {
                    playVideo(fieldValue); // Play video only if valid input is present
                  } else {
                    toast.error("Please enter a valid video ID.","Enter");
                  }
                }}
              >
                Play
              ...
              <Button
  onClick={() => {
    toast.error("Testing error toast!");
  }}
>
  Test Toast
</Button>
